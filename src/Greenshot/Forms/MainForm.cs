...
namespace Greenshot.Forms
{
    ...
    {
        private static readonly List<Tuple<string, int>> macrosToRun = new List<Tuple<string, int>>();

        private async void ExecuteMacros()
        {
            foreach (var macro in macrosToRun)
            {
                if (macro.Item2 > 0)
                {
                    LOG.InfoFormat("Waiting {0} seconds before running macro: {1}", macro.Item2, macro.Item1);
                    await Task.Delay(macro.Item2 * 1000);
                }
                MacroEngine.Run(macro.Item1);
            }
            macrosToRun.Clear();
        }
            ...
            try
            {
                ...
                for (int argumentNr = 0; argumentNr < arguments.Length; argumentNr++)
                {
                    ...
                    if (argument.ToLower().Equals("/help") || argument.ToLower().Equals("/h") || argument.ToLower().Equals("/?"))
                    {
                        ...
                        helpOutput.AppendLine();
                        helpOutput.AppendLine();
                        helpOutput.AppendLine("\t/macro [path to json]");
                        helpOutput.AppendLine("\t\tExecute a macro script from a JSON file.");
                        helpOutput.AppendLine();
                        helpOutput.AppendLine();
                        helpOutput.AppendLine("\t/macro-delay [seconds] [path to json]");
                        helpOutput.AppendLine("\t\tExecute a macro script after a specified delay.");
                        helpOutput.AppendLine();
                        helpOutput.AppendLine();
                    ...
                    if (argument.ToLower().Equals("/macro"))
                    {
                        if (argumentNr + 1 < arguments.Length)
                        {
                            string macroPath = arguments[++argumentNr];
                            if (isAlreadyRunning)
                            {
                                SendData(new CopyDataTransport(CommandEnum.RunMacro, Path.GetFullPath(macroPath)));
                                FreeMutex();
                                return;
                            }
                            macrosToRun.Add(new Tuple<string, int>(Path.GetFullPath(macroPath), 0));
                        }
                        continue;
                    }

                    if (argument.ToLower().Equals("/macro-delay"))
                    {
                        if (argumentNr + 2 < arguments.Length)
                        {
                            int delay = int.Parse(arguments[++argumentNr]);
                            string macroPath = arguments[++argumentNr];
                            if (isAlreadyRunning)
                            {
                                SendData(new CopyDataTransport(CommandEnum.RunMacro, delay + "|" + Path.GetFullPath(macroPath)));
                                FreeMutex();
                                return;
                            }
                            macrosToRun.Add(new Tuple<string, int>(Path.GetFullPath(macroPath), delay));
                        }
                        continue;
                    }
            ...
            foreach (KeyValuePair<CommandEnum, string> command in dataTransport.Commands)
            {
                ...
                switch (command.Key)
                {
                    ...
                    case CommandEnum.ReloadConfig:
                        ...
                        break;
                    case CommandEnum.RunMacro:
                        string fullCommand = command.Value;
                        int delayVal = 0;
                        string finalPath = fullCommand;
                        if (fullCommand.Contains("|"))
                        {
                            var parts = fullCommand.Split('|');
                            if (parts.Length == 2 && int.TryParse(parts[0], out delayVal))
                            {
                                finalPath = parts[1];
                            }
                        }

                        LOG.InfoFormat("Run macro requested: {0}, delay: {1}s", finalPath, delayVal);
                        BeginInvoke((MethodInvoker) async delegate { 
                            if (delayVal > 0) await Task.Delay(delayVal * 1000);
                            MacroEngine.Run(finalPath); 
                        });
                        break;
        ...
        private void Contextmenu_MacroEditorClick(object sender, EventArgs e)
        {
            using (var editor = new MacroEditorForm())
            {
                editor.ShowDialog();
            }
        }
